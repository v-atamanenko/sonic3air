...
{
	bool3 res = greaterThan(abs((yuv1 + yuv_offset) - (yuv2 + yuv_offset)), yuv_threshold);
...
void main()
{
	...
	vec3 w1 = yuv_matrix * texture2D(Texture, vTexCoord[1].xw).rgb;
	vec3 w2 = yuv_matrix * texture2D(Texture, vTexCoord[1].yw).rgb;
	vec3 w3 = yuv_matrix * texture2D(Texture, vTexCoord[1].zw).rgb;

	vec3 w4 = yuv_matrix * texture2D(Texture, vTexCoord[2].xw).rgb;
	vec3 w5 = yuv_matrix * p1;
	vec3 w6 = yuv_matrix * texture2D(Texture, vTexCoord[2].zw).rgb;

	vec3 w7 = yuv_matrix * texture2D(Texture, vTexCoord[3].xw).rgb;
	vec3 w8 = yuv_matrix * texture2D(Texture, vTexCoord[3].yw).rgb;
	vec3 w9 = yuv_matrix * texture2D(Texture, vTexCoord[3].zw).rgb;
